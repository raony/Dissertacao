...
A seleção de característica (\textit{feature selection}), na aprendizagem de
máquina, é a tarefa de escolher quais variáveis serão consideradas no
treinamento do sistema de forma a obter a melhor aproximação do modelo
\cite{Jain1997} \cite{Blum1997} \cite{Jain2000}. De forma similar devemos
ser capazes de selecionar redes que maximizem nossa análise de influência.
Recentemente \cite{Peng2005} sugere que as variáveis sejam escolhidas de forma a
maximizar a relevância e reduzir a redundância, mRMR
(\textit{minimal-redundancy-maximal-relevance}). No nosso caso, não conhecemos o
modelo da rede \textit{a priori} e a inferência é não-supervisionada. Por isso,
não temos como avaliar a relevância de uma rede em relação a outra, todas são
relevantes. Poderíamos considerar que redes que não demonstrem características
de redes sociais poderiam ser vistas como fortemente enviesadas e por isso de
baixa relevância, a saber: diâmetro pequeno (\textit{small world})
\cite{Milgram1967} \cite{Watts1998}, poucos muito conectados e muitos pouco
conectados (\textit{power-law}) \cite{Liljeros2001} \cite{Mitzenmacher2004} e
atores com muitas conexões tendem a estar conectados uns aos outros
(\textit{scale-free}) \cite{Li2005}.
...
\subsubsection{$\bigotimes$ Mínima redundância}
...
\subsubsection{Redundância das conexões}
...
\subsubsection{$\bigotimes$ Redes discretas e/ou esparsas}

O primeiro é mais apropriado para redes binárias ou com valores discretos
\cite{Dekker2007}. Consiste em criar um modelo exponencial para a criação de
grafos aleatórios a partir de um conjunto de parâmetros relacionados a
\textbf{configurações} de interesse \cite{ROBINS2007a}. Uma configuração pode ser
desde a presença de uma conexão, até a quantidade de k-triângulos e outros
...
\cite{ROBINS2007b} \cite{Snijders2006}.
...
Onde (i) $sim(.)$ é a similaridade de Jaccard (\cite{Jaccard1912}; \textit{Apud}
\cite{Berger-Wolf2006}), utilizada para comparar redes sociais e definida como
...
Substituindo (\ref{def:MI_discreto}) em (\ref{def:min_redun}) temos um modelo
para o conjunto de redes discretas com mínima redundância.
...
\cite{Anderson2001} \cite{Dekker2007}. Não é nosso objetivo nos aprofundar na
especifidades do teste, apenas é pertinente considerarmos a  utilização da
correlação linear $\alpha$ como valor para a informação mútua em
(\ref{def:min_redun}) para redes de valores contínuos densos.

\section{Mineração dos dados ou análise da influência}
...
\cite{Kirchhoff2009}. Abordagens recentes focam em modelos probabilísticos e
...
reconhecer atores chave. Em \cite{Kempe2003} encontramos uma comparação
...
\subsubsection{Tipo de envolvimento} 
...
\subsubsection{Propriedade do caminho}
...
será sua influência sobre este \cite{Watts2007}. Daí temos, naturalmente, a
...
\subsubsection{Redundância na proeminência}

Vimos na seção \ref{sec:criterios} como calcular a redundância das conexões entre
...
\subsubsection{Relevância na proeminência}

Quando tudo o que temos é a rede, para medirmos proeminência dos atores
precisamos ser capazes também de medir sua aplicabilidade. Vimos que
para alguns tipos de métricas a rede precisa aprensetar determinado padrão de
aglomeração. Por esta razão, podemos saber \textit{a priori} quando uma
representação vai produzir métricas significativas de proeminência. É sabido que
redes sociais são formadas por diversas comunidades sobrepostas
\cite{Palla2005} e por isso uma representação com alto grau de núcleo/periferia
nessas condições é pouco provável. Uma abordagem seria quebrar a rede em suas
comunidades para a partir daí encontrar os atores centrais de cada uma, existem
diversas ferramentas de decomposição da rede em \textit{k-cliques},
\textit{k-plexes}, \textit{k-cores} para grafos valorados e
direcionados (\cite{Peay1975}, \cite{Doreian1969}, \cite{Freeman1992};
\textit{Apud} \cite{Wasserman}), mas todas elas tendem a ignorar atores
pendentes que compõem grande parte da periferia. 

Um modelo mais interessante para o nosso problema é a chamada comunidade de
prática. A teoria das comunidades de prática não nasceu no meio sociométrico e
é voltada ao estudo da gestão do conhecimento, porém o trabalho de Schenkel
\cite{Schenkel2002} revisa as características estruturais típicas das
...
\textit{small-worlds}, cujo crescimento é logarítmico em relação ao crescimento
da rede;
\item[Núcleo/Periferia] A rede exibe um padrão de núcleo periferia, isto é, alto
grau de \textit{scale-freeness}.
\end{description}
...
\cite{Wenger1999}. Essas situações de trabalho, no entanto, podem ser
...
através de narrativas, conversação, \textit{mentoring} e aprendizagem por
experimentos \cite{Brown1991} \cite{Lave1991a}, tais atividades podem ser
transferidas para o meio digital \cite{Hildreth1998} \cite{Kimble2001} e deixam
rastros que podem ser mensurados para reconstruir a rede \cite{Tyler2005}
\cite{Welser2007}.
...
a palavra \textit{coletividade} quando nos referirmos à comunidade virtual na
...
algoritmos como os descritos em \cite{Palla2005} para reconhecer coletividades
...
aparecem em seus textos \cite{Reichling2005}, até criar uma ontologia dos termos
propriamente dito e a partir daí estimar uma conexão entre os atores
\cite{Mori2006}. Em \cite{Spertus2005} os autores propõem o uso da métrica de
similaridade para termos extraídos do texto TF-IDF \cite{Salton1989}
\cite{Frakes1992} e em \cite{MATSUO2007} encontramos uma versão aperfeiçoada que
diminui a necessidade de um \textit{corpus} da linguagem. A similaridade, dessa
forma, pode indicar uma homofilia de interesses quando cruzada com as
representações mensuradas das interações, essa homofilia poderia ser usada então
para filtrar a rede de atores e, em verificando as características de comunidades
de prática, também vir a aumentar a relevância da representação na análise da
influência.
